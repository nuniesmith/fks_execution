...
#[derive(Parser, Debug)]
#[command(version, about="FKS Execution API")] struct Cli { #[arg(long, default_value="0.0.0.0:8090")] listen: String }

#[derive(Serialize)] struct Signal { symbol: String, rsi: f64, ema: f64, risk_allowance: f64, latency_ms: u128 }

#[tokio::main]
async fn main() -> anyhow::Result<()> { tracing_subscriber::fmt::init(); let cli = Cli::parse(); let app = Router::new().route("/execute/signal", get(signal_handler)); let addr: SocketAddr = cli.listen.parse()?; tracing::info!(%addr, "execution api listening"); axum::serve(tokio::net::TcpListener::bind(addr).await?, app).await?; Ok(()) }
